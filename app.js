  ...
  if (state.teams.length === 0) {
    ...
    state.teams.forEach(t => {
      html += `<div class="row">
        ...
        <button data-action="rename-team" data-id="${t.id}" class="ghost">Renommer</button>
        <button data-action="delete-team" data-id="${t.id}" class="ghost">Supprimer</button>
      </div>`;
    });
  }
...
function renameTeamModal(teamId) {
  const team = state.teams.find(t => t.id === teamId);
  openModal('Renommer l\'équipe', `
    <label>Nom de l'équipe</label>
    <input type="text" id="m-team-rename" value="${escapeAttr(team.name)}">
  `, async (overlay) => {
    const name = overlay.querySelector('#m-team-rename').value.trim();
    if (!name) return;
    const { error } = await sb.from('teams').update({ name }).eq('id', teamId);
    if (error) { alert('Erreur : ' + error.message); return; }
    team.name = name;
    renderAccueil();
  });
...
function newMatchModal() {
  ...
  `, async (overlay) => {
    ...
    const draft = {
      teamId: state.currentTeamId, opponent, date,
    ...
    const row = matchToRow(draft);
    delete row.id; // laisse la base de données générer l'identifiant
...
document.addEventListener('click', async (e) => {
  ...
  if (action === 'rename-team') renameTeamModal(id);
  if (action === 'delete-team') {
    const team = state.teams.find(t => t.id === id);
    if (confirm(`Supprimer l'équipe "${team.name}" ? Ses joueurs, matchs et tâches seront aussi supprimés.`)) {
      const { error } = await sb.from('teams').delete().eq('id', id);
      if (error) { alert('Erreur : ' + error.message); return; }
      state.teams = state.teams.filter(t => t.id !== id);
      state.players = state.players.filter(p => p.teamId !== id);
      state.matches = state.matches.filter(m => m.teamId !== id);
      state.tasks = state.tasks.filter(t => t.teamId !== id);
      if (state.currentTeamId === id) state.currentTeamId = state.teams[0]?.id || null;
      renderAccueil();
    }
  }
